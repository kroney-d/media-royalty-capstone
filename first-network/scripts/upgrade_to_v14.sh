...
CC_SRC_PATH="github.com/chaincode/chaincode_mediacoin02/go/"
if [ "$LANGUAGE" = "node" ]; then
  CC_SRC_PATH="/opt/gopath/src/github.com/chaincode/chaincode_mediacoin02/node/"
fi
...
addCapabilityToChannel() {
        ...
        if [ $CH_NAME != $SYS_CHANNEL ] ; then
                if [ $GROUP == "orderer" ]; then
                      ...
                      # Sign with Peerartist.Admin
                      signConfigtxAsPeerOrg 1 config_update_in_envelope.pb
                      # Prepare to sign the update as the Peerbuyer.Admin
                      setGlobals 0 2
                fi
        else
        ...
        if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
                set -x
                peer channel update -f config_update_in_envelope.pb -c $CH_NAME -o orderer.mediacoin.com:7050 --cafile $ORDERER_CA
                res=$?
                set +x
        else
                set -x
                peer channel update -f config_update_in_envelope.pb -c $CH_NAME -o orderer.mediacoin.com:7050 --tls true --cafile $ORDERER_CA
                res=$?
                set +x
        fi
...
#Query on chaincode on Peer0/artist
echo "Querying chaincode on artist/peer0..."
chaincodeQuery 0 1 90

sleep $DELAY

#Invoke on chaincode on Peer0/artist
echo "Sending invoke transaction on artist/peer0..."
